...
void assign_work(void *arg) {
	...
	if (!server) {
		log_to_file(log_file, "Error: Couldn't cast void *arg to server_t in assign_work()");
		return;
	}
...
server_t *server_create(bool daemon, size_t port, size_t request_handling, char *log) {
	...
	log_to_file(log_file, "Server initialized with %ld threads", nr_of_threads);
...
void server_listen(server_t *server) {
	if (listen(server->socket, 30) != 0)
		log_to_file(log_file, "Error: Couldn't listen() on port %ld in server_listen()", server->port);

	printf("Listening on port %ld...\n", server->port);
	log_to_file(log_file, "Server listening on port %ld...\n", server->port);
	...
	while (true) {
		...
		if (select(FD_SETSIZE, &ready_sockets, NULL, NULL, NULL) < 0) // check socket descriptors
			log_to_file(log_file, "Error: Couldn't select() in server_listen()");
		...
		for (size_t i = 0; i <= max_socket; i++) {
			...
			if (new_socket == server->socket) // new connection
			{
				...
				if ((new_socket = accept(server->socket, (struct sockaddr *)&server->storage, &server->address_size)) == -1) {
					log_to_file(log_file, "Error: Couldn't accept() in server_listen()");
					continue;
				}
				...
				log_to_file(log_file, "Accepted new connection from %s\n", get_ip_from_socket_fd(new_socket));
				continue;
			}
			...
			{
				log_to_file(log_file, "Error: Couldn't recv() in server_listen()");
				continue;
			}
